...
async function searchOSM(lat, lng, type) {
    ...
    if (data && data.elements) {
        for (const el of data.elements) {
            ...
            if (!addr) addr = [el.tags['contact:city'] || el.tags['is_in:city'], el.tags.is_in].filter(Boolean).join(', ');
...
app.get('/api/doctors', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: "Failed to fetch doctors", results: [], source: 'error' });
    }
});
